...
<%--
/*
* fmiOutput.jsp    1.1.0  2016-07-24
*  
...
<%
  ...
  FMIOutput fmiOutput = Domain2OSLCManager.getFMIOutputMapByIdentifier(fmuFileName, identifier);
  ...
  String outputURI = request.getScheme() + "://" + request.getServerName() + ":" + request.getServerPort() + 
    "/oslc4jfmi/services/" + fmuFileName + "/Output/" + identifier;
%>
...
<h4>Attributes</h4> 
  ...
  <%= fmiOutput.getIdentifier() %>&nbsp;</div><hr>
  <%  } %>
  <%  if (fmiOutput != null) { %>
  <div class="col-md-12"><b>Name</b><br>
  <%= fmiOutput.getName() %>&nbsp;</div><hr>
  <%  } %>
  <%  if (fmiOutput != null) { %>
  <div class="col-md-12"><b>Index</b><br>
  <%= fmiOutput.getIndex() %>&nbsp;</div><hr>
  <%  } %>
  <%  if (fmiOutput != null) { %>
  <div class="col-md-12"><b>Dependencies</b><br>
  <%= fmiOutput.getDependencies() %>&nbsp;</div><hr>
  <%  } %>
  <%  if (fmiOutput != null) { %>
  <div class="col-md-12"><b>DependenciesKind</b><br>
  <%= fmiOutput.getDependenciesKind() %>&nbsp;</div><hr>
  <%  } %>
  </div>
</body>
</html>
