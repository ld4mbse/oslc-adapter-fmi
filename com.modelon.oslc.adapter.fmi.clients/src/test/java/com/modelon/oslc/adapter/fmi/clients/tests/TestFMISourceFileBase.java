/*
* TestFMISourceFileBase.java    1.1.0  2016-07-23
*  
...
public class TestFMISourceFileBase {
  ...
  protected TestFMISourceFileBase() throws Exception {
    super();
  }

  private static void verifyFMISourceFile(final String   mediaType, final FMISourceFile theFMISourceFile) {
    System.out.println("verifyFMISourceFile");
    assertNotNull(theFMISourceFile);
 
    final URI  aboutURI     = theFMISourceFile.getAbout();
    final String identifier  = theFMISourceFile.getIdentifier();
    final String file  = theFMISourceFile.getFile();

    System.out.println("SourceFile Query Capability : " + aboutURI);
  ...
  protected void testRetrieves(final String mediaType) {
    ...
    for (ServiceProvider serviceProvider : oslcServiceProviderCatalog.getServiceProviders()) {
      ...
      for (final Service service : services) {
        ...
        if (Constants.FMI_SOURCEFILE_DOMAIN.equals(String.valueOf(service.getDomain()))) {
          ...
          for (final QueryCapability queryCapability : queryCapabilities) {
            ...
            final FMISourceFile[] theFMISourceFileArray = oslcRestClient.getOslcResources(FMISourceFile[].class);
            
            
            if (theFMISourceFileArray != null) {
              if (theFMISourceFileArray.length < 1) {
                System.out.println("    No FMISourceFile Resources");
              }
              for (final FMISourceFile theFMISourceFile : theFMISourceFileArray) {
                verifyFMISourceFile(mediaType, theFMISourceFile);
              }
            } else { 
              System.out.println("    No FMISourceFile Resources");
            } 
