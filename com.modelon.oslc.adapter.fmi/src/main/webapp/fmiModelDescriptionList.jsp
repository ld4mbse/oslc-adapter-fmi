...
<%--
/*
* fmiModelDescriptionList.jsp    1.1.0  2016-07-24
*  
...
<%
  ...
  List<FMIModelDescription> fmiModelDescriptionList = null;
  if (identifier != null) {
    ...
    fmiModelDescriptionList = obj;
  } else { 
    fmiModelDescriptionList = Domain2OSLCManager.getFMIModelDescriptionMap(fmuFileName);
  }
  String modeldescriptionURI = request.getScheme() + "://" + request.getServerName() + ":" + request.getServerPort() + 
    "/oslc4jfmi/services/Resource/" + fmuFileName + "/ModelDescription";
%>
...
<%  if (fmiModelDescriptionList != null && fmiModelDescriptionList.size() > 0) { %>
      <% for (FMIModelDescription fmiModelDescription : fmiModelDescriptionList) { %>
        <div class="col-md-12"><h4>ModelDescription : <%= Domain2OSLCManager.decodeURL(Domain2OSLCManager.getVariableName(fmiModelDescription.getAbout().toString())) %></h4><a href="<%= fmiModelDescription.getAbout().toString() %>"><%= fmiModelDescription.getAbout().toString() %></a></div>
      <% } %> 
<%  } else { %> 
      <b>No resources</b>
<%  } %>
