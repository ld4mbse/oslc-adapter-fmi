...
<%--
/*
* fmiModelStructure.jsp    1.1.0  2016-07-24
*  
...
<%
  ...
  FMIModelStructure fmiModelStructure = Domain2OSLCManager.getFMIModelStructureMapByIdentifier(fmuFileName, identifier);
  ...
  String modelstructureURI = request.getScheme() + "://" + request.getServerName() + ":" + request.getServerPort() + 
    "/oslc4jfmi/services/" + fmuFileName + "/ModelStructure/" + identifier;
%>
...
<h4>ResourceLinks</h4> 
  <div class="container">
  <% if (fmiModelStructure != null && fmiModelStructure.getDerivatives() != null) { %>
  <div class="col-md-12"><h4>Derivatives</h4>
    <div class="container">
<% for (Link link : fmiModelStructure.getDerivatives()) { %>
...
<% } %>
  <% if (fmiModelStructure != null && fmiModelStructure.getInitialUnknowns() != null) { %>
  <div class="col-md-12"><h4>InitialUnknowns</h4>
    <div class="container">
<% for (Link link : fmiModelStructure.getInitialUnknowns()) { %>
        <div class="col-md-12"><b><%= Domain2OSLCManager.decodeURL(Domain2OSLCManager.getVariableName(link.getValue().toString())) %></b><br>
        <a href="<%= link.getValue().toString() %>"><%= link.getValue().toString() %></a></div>
      <% } %>
  </div></div>
<% } %>
  <% if (fmiModelStructure != null && fmiModelStructure.getOutputs() != null) { %>
  <div class="col-md-12"><h4>Outputs</h4>
    <div class="container">
<% for (Link link : fmiModelStructure.getOutputs()) { %>
        <div class="col-md-12"><b><%= Domain2OSLCManager.decodeURL(Domain2OSLCManager.getVariableName(link.getValue().toString())) %></b><br>
        <a href="<%= link.getValue().toString() %>"><%= link.getValue().toString() %></a></div>
      <% } %>
  </div></div>
...
<h4>Attributes</h4> 
  <div class="container">
  <%  if (fmiModelStructure != null) { %>
  <div class="col-md-12"><b>Identifier</b><br>
  <%= fmiModelStructure.getIdentifier() %>&nbsp;</div><hr>
  <%  } %>
  </div>
</body>
</html>
