...
<%--
/*
* fmiAnnotationList.jsp    1.1.0  2016-07-24
*  
...
<%
  ...
  List<FMIAnnotation> fmiAnnotationList = null;
  if (identifier != null) {
    ...
    fmiAnnotationList = obj;
  } else { 
    fmiAnnotationList = Domain2OSLCManager.getFMIAnnotationMap(fmuFileName);
  }
  String annotationURI = request.getScheme() + "://" + request.getServerName() + ":" + request.getServerPort() + 
    "/oslc4jfmi/services/Resource/" + fmuFileName + "/Annotation";
%>
...
<%  if (fmiAnnotationList != null && fmiAnnotationList.size() > 0) { %>
      <% for (FMIAnnotation fmiAnnotation : fmiAnnotationList) { %>
        <div class="col-md-12"><h4>Annotation : <%= Domain2OSLCManager.decodeURL(Domain2OSLCManager.getVariableName(fmiAnnotation.getAbout().toString())) %></h4><a href="<%= fmiAnnotation.getAbout().toString() %>"><%= fmiAnnotation.getAbout().toString() %></a></div>
      <% } %> 
<%  } else { %> 
      <b>No resources</b>
<%  } %>
