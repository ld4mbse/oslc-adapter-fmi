...
<%--
/*
* fmiEnumerationList.jsp    1.1.0  2016-07-24
*  
...
<%
  ...
  List<FMIEnumeration> fmiEnumerationList = null;
  if (identifier != null) {
    ...
    fmiEnumerationList = obj;
  } else { 
    fmiEnumerationList = Domain2OSLCManager.getFMIEnumerationMap(fmuFileName);
  }
  String enumerationURI = request.getScheme() + "://" + request.getServerName() + ":" + request.getServerPort() + 
    "/oslc4jfmi/services/Resource/" + fmuFileName + "/Enumeration";
%>
...
<%  if (fmiEnumerationList != null && fmiEnumerationList.size() > 0) { %>
      <% for (FMIEnumeration fmiEnumeration : fmiEnumerationList) { %>
        <div class="col-md-12"><h4>Enumeration : <%= Domain2OSLCManager.decodeURL(Domain2OSLCManager.getVariableName(fmiEnumeration.getAbout().toString())) %></h4><a href="<%= fmiEnumeration.getAbout().toString() %>"><%= fmiEnumeration.getAbout().toString() %></a></div>
      <% } %> 
<%  } else { %> 
      <b>No resources</b>
<%  } %>
