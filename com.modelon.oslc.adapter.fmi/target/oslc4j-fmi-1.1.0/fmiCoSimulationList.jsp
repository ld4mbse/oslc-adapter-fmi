...
<%--
/*
* fmiCoSimulationList.jsp    1.1.0  2016-07-24
*  
...
<%
  ...
  List<FMICoSimulation> fmiCoSimulationList = null;
  if (identifier != null) {
    ...
    fmiCoSimulationList = obj;
  } else { 
    fmiCoSimulationList = Domain2OSLCManager.getFMICoSimulationMap(fmuFileName);
  }
  String cosimulationURI = request.getScheme() + "://" + request.getServerName() + ":" + request.getServerPort() + 
    "/oslc4jfmi/services/Resource/" + fmuFileName + "/CoSimulation";
%>
...
<%  if (fmiCoSimulationList != null && fmiCoSimulationList.size() > 0) { %>
      <% for (FMICoSimulation fmiCoSimulation : fmiCoSimulationList) { %>
        <div class="col-md-12"><h4>CoSimulation : <%= Domain2OSLCManager.decodeURL(Domain2OSLCManager.getVariableName(fmiCoSimulation.getAbout().toString())) %></h4><a href="<%= fmiCoSimulation.getAbout().toString() %>"><%= fmiCoSimulation.getAbout().toString() %></a></div>
      <% } %> 
<%  } else { %> 
      <b>No resources</b>
<%  } %>
