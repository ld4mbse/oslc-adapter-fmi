/*
* TestFMIModelStructureBase.java    1.1.0  2016-07-23
*  
...
public class TestFMIModelStructureBase {
  ...
  protected TestFMIModelStructureBase() throws Exception {
    super();
  }

  private static void verifyFMIModelStructure(final String   mediaType, final FMIModelStructure theFMIModelStructure) {
    System.out.println("verifyFMIModelStructure");
    assertNotNull(theFMIModelStructure);
 
    final URI  aboutURI     = theFMIModelStructure.getAbout();
    final String identifier  = theFMIModelStructure.getIdentifier();

    System.out.println("ModelStructure Query Capability : " + aboutURI);
  ...
  protected void testRetrieves(final String mediaType) {
    ...
    for (ServiceProvider serviceProvider : oslcServiceProviderCatalog.getServiceProviders()) {
      ...
      for (final Service service : services) {
        ...
        if (Constants.FMI_MODELSTRUCTURE_DOMAIN.equals(String.valueOf(service.getDomain()))) {
          ...
          for (final QueryCapability queryCapability : queryCapabilities) {
            ...
            final FMIModelStructure[] theFMIModelStructureArray = oslcRestClient.getOslcResources(FMIModelStructure[].class);
            
            
            if (theFMIModelStructureArray != null) {
              if (theFMIModelStructureArray.length < 1) {
                System.out.println("    No FMIModelStructure Resources");
              }
              for (final FMIModelStructure theFMIModelStructure : theFMIModelStructureArray) {
                verifyFMIModelStructure(mediaType, theFMIModelStructure);
              }
            } else { 
              System.out.println("    No FMIModelStructure Resources");
            } 
