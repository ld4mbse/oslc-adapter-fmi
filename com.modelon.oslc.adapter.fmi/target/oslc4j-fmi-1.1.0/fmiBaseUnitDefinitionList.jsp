...
<%--
/*
* fmiBaseUnitDefinitionList.jsp    1.1.0  2016-07-24
*  
...
<%@ page import="org.eclipse.lyo.oslc4j.core.model.ServiceProvider"%>
<%@ page import="org.eclipse.lyo.oslc4j.core.model.Link"%>
<%@ page import="com.modelon.oslc.adapter.fmi.resources.FMIBaseUnitDefinition" %>
...
<%
  ...
  List<FMIBaseUnitDefinition> fmiBaseUnitDefinitionList = null;
  if (identifier != null) {
    ...
    fmiBaseUnitDefinitionList = obj;
  } else { 
    fmiBaseUnitDefinitionList = Domain2OSLCManager.getFMIBaseUnitDefinitionMap(fmuFileName);
  }
  String baseunitdefinitionURI = request.getScheme() + "://" + request.getServerName() + ":" + request.getServerPort() + 
    "/oslc4jfmi/services/Resource/" + fmuFileName + "/BaseUnitDefinition";
%>
...
<div class="row" style="z-index:100; position: fixed;top:0;"><ol class="breadcrumb">
  ...
  <li class="active"><b>BaseUnitDefinition Query Capability</b></li>
</ol></div>

<div class="row alert alert-success">
  <h1 style="margin-top:0px;">BaseUnitDefinition Query Capability</h1>
  <p class="lead"><b><%= fmuFileName %></b></p>
  This document: <a href="<%= baseunitdefinitionURI %>"> <%= baseunitdefinitionURI %></a>
</div>

<%  if (fmiBaseUnitDefinitionList != null && fmiBaseUnitDefinitionList.size() > 0) { %>
      <% for (FMIBaseUnitDefinition fmiBaseUnitDefinition : fmiBaseUnitDefinitionList) { %>
        <div class="col-md-12"><h4>BaseUnitDefinition : <%= Domain2OSLCManager.decodeURL(Domain2OSLCManager.getVariableName(fmiBaseUnitDefinition.getAbout().toString())) %></h4><a href="<%= fmiBaseUnitDefinition.getAbout().toString() %>"><%= fmiBaseUnitDefinition.getAbout().toString() %></a></div>
      <% } %> 
<%  } else { %> 
      <b>No resources</b>
<%  } %>
