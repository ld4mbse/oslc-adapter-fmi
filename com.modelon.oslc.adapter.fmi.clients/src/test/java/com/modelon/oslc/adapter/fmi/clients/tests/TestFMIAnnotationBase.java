/*
* TestFMIAnnotationBase.java    1.1.0  2016-07-23
*  
...
public class TestFMIAnnotationBase {
  ...
  protected TestFMIAnnotationBase() throws Exception {
    super();
  }

  private static void verifyFMIAnnotation(final String   mediaType, final FMIAnnotation theFMIAnnotation) {
    System.out.println("verifyFMIAnnotation");
    assertNotNull(theFMIAnnotation);
 
    final URI  aboutURI     = theFMIAnnotation.getAbout();
    final String identifier  = theFMIAnnotation.getIdentifier();
    final String name  = theFMIAnnotation.getName();
    final String value  = theFMIAnnotation.getValue();

    System.out.println("Annotation Query Capability : " + aboutURI);
  ...
  protected void testRetrieves(final String mediaType) {
    ...
    for (ServiceProvider serviceProvider : oslcServiceProviderCatalog.getServiceProviders()) {
      ...
      for (final Service service : services) {
        ...
        if (Constants.FMI_ANNOTATION_DOMAIN.equals(String.valueOf(service.getDomain()))) {
          ...
          for (final QueryCapability queryCapability : queryCapabilities) {
            ...
            final FMIAnnotation[] theFMIAnnotationArray = oslcRestClient.getOslcResources(FMIAnnotation[].class);
            
            
            if (theFMIAnnotationArray != null) {
              if (theFMIAnnotationArray.length < 1) {
                System.out.println("    No FMIAnnotation Resources");
              }
              for (final FMIAnnotation theFMIAnnotation : theFMIAnnotationArray) {
                verifyFMIAnnotation(mediaType, theFMIAnnotation);
              }
            } else { 
              System.out.println("    No FMIAnnotation Resources");
            } 
