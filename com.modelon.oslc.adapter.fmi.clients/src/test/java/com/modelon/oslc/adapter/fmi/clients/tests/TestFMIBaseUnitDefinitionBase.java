/*
* TestFMIBaseUnitDefinitionBase.java    1.1.0  2016-07-23
*  
...
public class TestFMIBaseUnitDefinitionBase {
  ...
  protected TestFMIBaseUnitDefinitionBase() throws Exception {
    super();
  }

  private static void verifyFMIBaseUnitDefinition(final String   mediaType, final FMIBaseUnitDefinition theFMIBaseUnitDefinition) {
    System.out.println("verifyFMIBaseUnitDefinition");
    assertNotNull(theFMIBaseUnitDefinition);
 
    final URI  aboutURI     = theFMIBaseUnitDefinition.getAbout();
    final String identifier  = theFMIBaseUnitDefinition.getIdentifier();
    final String displayUnit  = theFMIBaseUnitDefinition.getDisplayUnit();

    System.out.println("BaseUnitDefinition Query Capability : " + aboutURI);
  ...
  protected void testRetrieves(final String mediaType) {
    ...
    for (ServiceProvider serviceProvider : oslcServiceProviderCatalog.getServiceProviders()) {
      ...
      for (final Service service : services) {
        ...
        if (Constants.FMI_BASEUNITDEFINITION_DOMAIN.equals(String.valueOf(service.getDomain()))) {
          ...
          for (final QueryCapability queryCapability : queryCapabilities) {
            ...
            final FMIBaseUnitDefinition[] theFMIBaseUnitDefinitionArray = oslcRestClient.getOslcResources(FMIBaseUnitDefinition[].class);
            
            
            if (theFMIBaseUnitDefinitionArray != null) {
              if (theFMIBaseUnitDefinitionArray.length < 1) {
                System.out.println("    No FMIBaseUnitDefinition Resources");
              }
              for (final FMIBaseUnitDefinition theFMIBaseUnitDefinition : theFMIBaseUnitDefinitionArray) {
                verifyFMIBaseUnitDefinition(mediaType, theFMIBaseUnitDefinition);
              }
            } else { 
              System.out.println("    No FMIBaseUnitDefinition Resources");
            } 
