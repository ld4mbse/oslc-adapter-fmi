/*
* TestFMIListingAliasBase.java    1.1.0  2016-07-23
*  
...
public class TestFMIListingAliasBase {
  ...
  protected TestFMIListingAliasBase() throws Exception {
    super();
  }

  private static void verifyFMIListingAlias(final String   mediaType, final FMIListingAlias theFMIListingAlias) {
    System.out.println("verifyFMIListingAlias");
    assertNotNull(theFMIListingAlias);
 
    final URI  aboutURI     = theFMIListingAlias.getAbout();
    final String identifier  = theFMIListingAlias.getIdentifier();
    final String name  = theFMIListingAlias.getName();

    System.out.println("ListingAlias Query Capability : " + aboutURI);
  ...
  protected void testRetrieves(final String mediaType) {
    ...
    for (ServiceProvider serviceProvider : oslcServiceProviderCatalog.getServiceProviders()) {
      ...
      for (final Service service : services) {
        ...
        if (Constants.FMI_LISTINGALIAS_DOMAIN.equals(String.valueOf(service.getDomain()))) {
          ...
          for (final QueryCapability queryCapability : queryCapabilities) {
            ...
            final FMIListingAlias[] theFMIListingAliasArray = oslcRestClient.getOslcResources(FMIListingAlias[].class);
            
            
            if (theFMIListingAliasArray != null) {
              if (theFMIListingAliasArray.length < 1) {
                System.out.println("    No FMIListingAlias Resources");
              }
              for (final FMIListingAlias theFMIListingAlias : theFMIListingAliasArray) {
                verifyFMIListingAlias(mediaType, theFMIListingAlias);
              }
            } else { 
              System.out.println("    No FMIListingAlias Resources");
            } 
