/*
* TestFMIFMUBase.java    1.1.0  2016-07-23
*  
...
public class TestFMIFMUBase {
  ...
  protected TestFMIFMUBase() throws Exception {
    super();
  }

  private static void verifyFMIFMU(final String   mediaType, final FMIFMU theFMIFMU) {
    System.out.println("verifyFMIFMU");
    assertNotNull(theFMIFMU);
 
    final URI  aboutURI     = theFMIFMU.getAbout();
    final String identifier  = theFMIFMU.getIdentifier();
    final String fmiVersion  = theFMIFMU.getFmiVersion();
    final String isModelExchangeFMU  = theFMIFMU.getIsModelExchangeFMU();
    final String isCoSimulationFMU  = theFMIFMU.getIsCoSimulationFMU();

    System.out.println("FMU Query Capability : " + aboutURI);
    System.out.println("    identifier = " + identifier);
    System.out.println("    fmiVersion = " + fmiVersion);
    System.out.println("    isModelExchangeFMU = " + isModelExchangeFMU);
    System.out.println("    isCoSimulationFMU = " + isCoSimulationFMU);
    
    assertTrue(aboutURI.toString().endsWith(identifier));
    assertNotNull(aboutURI);
    assertNotNull(identifier);
    assertNotNull(fmiVersion);
    assertNotNull(isModelExchangeFMU);
    assertNotNull(isCoSimulationFMU);
  }
  ...
  protected void testRetrieves(final String mediaType) {
    ...
    for (ServiceProvider serviceProvider : oslcServiceProviderCatalog.getServiceProviders()) {
      ...
      for (final Service service : services) {
        ...
        if (Constants.FMI_FMU_DOMAIN.equals(String.valueOf(service.getDomain()))) {
          ...
          for (final QueryCapability queryCapability : queryCapabilities) {
            ...
            final FMIFMU[] theFMIFMUArray = oslcRestClient.getOslcResources(FMIFMU[].class);
            
            
            if (theFMIFMUArray != null) {
              if (theFMIFMUArray.length < 1) {
                System.out.println("    No FMIFMU Resources");
              }
              for (final FMIFMU theFMIFMU : theFMIFMUArray) {
                verifyFMIFMU(mediaType, theFMIFMU);
              }
            } else { 
              System.out.println("    No FMIFMU Resources");
            } 
