/*
* TestFMIDefaultExperimentBase.java    1.1.0  2016-07-23
*  
...
public class TestFMIDefaultExperimentBase {
  ...
  protected TestFMIDefaultExperimentBase() throws Exception {
    super();
  }

  private static void verifyFMIDefaultExperiment(final String   mediaType, final FMIDefaultExperiment theFMIDefaultExperiment) {
    System.out.println("verifyFMIDefaultExperiment");
    assertNotNull(theFMIDefaultExperiment);
 
    final URI  aboutURI     = theFMIDefaultExperiment.getAbout();
    final String identifier  = theFMIDefaultExperiment.getIdentifier();
    final String startTime  = theFMIDefaultExperiment.getStartTime();
    final String stopTime  = theFMIDefaultExperiment.getStopTime();
    final String tolerance  = theFMIDefaultExperiment.getTolerance();
    final String step  = theFMIDefaultExperiment.getStep();

    System.out.println("DefaultExperiment Query Capability : " + aboutURI);
  ...
  protected void testRetrieves(final String mediaType) {
    ...
    for (ServiceProvider serviceProvider : oslcServiceProviderCatalog.getServiceProviders()) {
      ...
      for (final Service service : services) {
        ...
        if (Constants.FMI_DEFAULTEXPERIMENT_DOMAIN.equals(String.valueOf(service.getDomain()))) {
          ...
          for (final QueryCapability queryCapability : queryCapabilities) {
            ...
            final FMIDefaultExperiment[] theFMIDefaultExperimentArray = oslcRestClient.getOslcResources(FMIDefaultExperiment[].class);
            
            
            if (theFMIDefaultExperimentArray != null) {
              if (theFMIDefaultExperimentArray.length < 1) {
                System.out.println("    No FMIDefaultExperiment Resources");
              }
              for (final FMIDefaultExperiment theFMIDefaultExperiment : theFMIDefaultExperimentArray) {
                verifyFMIDefaultExperiment(mediaType, theFMIDefaultExperiment);
              }
            } else { 
              System.out.println("    No FMIDefaultExperiment Resources");
            } 
