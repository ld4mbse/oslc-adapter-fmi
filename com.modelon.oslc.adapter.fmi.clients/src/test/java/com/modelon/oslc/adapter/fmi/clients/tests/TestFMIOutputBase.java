/*
* TestFMIOutputBase.java    1.1.0  2016-07-23
*  
...
public class TestFMIOutputBase {
  ...
  protected TestFMIOutputBase() throws Exception {
    super();
  }

  private static void verifyFMIOutput(final String   mediaType, final FMIOutput theFMIOutput) {
    System.out.println("verifyFMIOutput");
    assertNotNull(theFMIOutput);
 
    final URI  aboutURI     = theFMIOutput.getAbout();
    final String identifier  = theFMIOutput.getIdentifier();
    final String name  = theFMIOutput.getName();
    final String index  = theFMIOutput.getIndex();
    final String dependencies  = theFMIOutput.getDependencies();
    final String dependenciesKind  = theFMIOutput.getDependenciesKind();

    System.out.println("Output Query Capability : " + aboutURI);
  ...
  protected void testRetrieves(final String mediaType) {
    ...
    for (ServiceProvider serviceProvider : oslcServiceProviderCatalog.getServiceProviders()) {
      ...
      for (final Service service : services) {
        ...
        if (Constants.FMI_OUTPUT_DOMAIN.equals(String.valueOf(service.getDomain()))) {
          ...
          for (final QueryCapability queryCapability : queryCapabilities) {
            ...
            final FMIOutput[] theFMIOutputArray = oslcRestClient.getOslcResources(FMIOutput[].class);
            
            
            if (theFMIOutputArray != null) {
              if (theFMIOutputArray.length < 1) {
                System.out.println("    No FMIOutput Resources");
              }
              for (final FMIOutput theFMIOutput : theFMIOutputArray) {
                verifyFMIOutput(mediaType, theFMIOutput);
              }
            } else { 
              System.out.println("    No FMIOutput Resources");
            } 
