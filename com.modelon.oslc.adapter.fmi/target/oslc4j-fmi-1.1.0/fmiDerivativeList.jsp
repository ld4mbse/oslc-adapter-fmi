...
<%--
/*
* fmiDerivativeList.jsp    1.1.0  2016-07-24
*  
...
<%@ page import="org.eclipse.lyo.oslc4j.core.model.ServiceProvider"%>
<%@ page import="org.eclipse.lyo.oslc4j.core.model.Link"%>
<%@ page import="com.modelon.oslc.adapter.fmi.resources.FMIDerivative" %>
...
<div class="row" style="z-index:100; position: fixed;top:0;"><ol class="breadcrumb">
  ...
  <li class="active"><b>Derivative Query Capability</b></li>
</ol></div>

<div class="row alert alert-success">
  <h1 style="margin-top:0px;">Derivative Query Capability</h1>
  <p class="lead"><b><%= fmuFileName %></b></p>
  This document: <a href="<%= derivativeURI %>"> <%= derivativeURI %></a>
</div>

<%  if (fmiDerivativeList != null && fmiDerivativeList.size() > 0) { %>
      <% for (FMIDerivative fmiDerivative : fmiDerivativeList) { %>
        <div class="col-md-12"><h4>Derivative : <%= Domain2OSLCManager.decodeURL(Domain2OSLCManager.getVariableName(fmiDerivative.getAbout().toString())) %></h4><a href="<%= fmiDerivative.getAbout().toString() %>"><%= fmiDerivative.getAbout().toString() %></a></div>
      <% } %> 
<%  } else { %> 
      <b>No resources</b>
<%  } %>
