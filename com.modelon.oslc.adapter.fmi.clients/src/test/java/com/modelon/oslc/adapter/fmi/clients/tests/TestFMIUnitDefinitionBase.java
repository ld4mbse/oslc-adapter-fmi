/*
* TestFMIUnitDefinitionBase.java    1.1.0  2016-07-23
*  
...
public class TestFMIUnitDefinitionBase {
  ...
  protected TestFMIUnitDefinitionBase() throws Exception {
    super();
  }

  private static void verifyFMIUnitDefinition(final String   mediaType, final FMIUnitDefinition theFMIUnitDefinition) {
    System.out.println("verifyFMIUnitDefinition");
    assertNotNull(theFMIUnitDefinition);
 
    final URI  aboutURI     = theFMIUnitDefinition.getAbout();
    final String identifier  = theFMIUnitDefinition.getIdentifier();
    final String name  = theFMIUnitDefinition.getName();

    System.out.println("UnitDefinition Query Capability : " + aboutURI);
  ...
  protected void testRetrieves(final String mediaType) {
    ...
    for (ServiceProvider serviceProvider : oslcServiceProviderCatalog.getServiceProviders()) {
      ...
      for (final Service service : services) {
        ...
        if (Constants.FMI_UNITDEFINITION_DOMAIN.equals(String.valueOf(service.getDomain()))) {
          ...
          for (final QueryCapability queryCapability : queryCapabilities) {
            ...
            final FMIUnitDefinition[] theFMIUnitDefinitionArray = oslcRestClient.getOslcResources(FMIUnitDefinition[].class);
            
            
            if (theFMIUnitDefinitionArray != null) {
              if (theFMIUnitDefinitionArray.length < 1) {
                System.out.println("    No FMIUnitDefinition Resources");
              }
              for (final FMIUnitDefinition theFMIUnitDefinition : theFMIUnitDefinitionArray) {
                verifyFMIUnitDefinition(mediaType, theFMIUnitDefinition);
              }
            } else { 
              System.out.println("    No FMIUnitDefinition Resources");
            } 
