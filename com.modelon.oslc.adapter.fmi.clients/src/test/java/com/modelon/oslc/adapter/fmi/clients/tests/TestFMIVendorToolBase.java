/*
* TestFMIVendorToolBase.java    1.1.0  2016-07-23
*  
...
public class TestFMIVendorToolBase {
  ...
  protected TestFMIVendorToolBase() throws Exception {
    super();
  }

  private static void verifyFMIVendorTool(final String   mediaType, final FMIVendorTool theFMIVendorTool) {
    System.out.println("verifyFMIVendorTool");
    assertNotNull(theFMIVendorTool);
 
    final URI  aboutURI     = theFMIVendorTool.getAbout();
    final String identifier  = theFMIVendorTool.getIdentifier();
    final String name  = theFMIVendorTool.getName();

    System.out.println("VendorTool Query Capability : " + aboutURI);
  ...
  protected void testRetrieves(final String mediaType) {
    ...
    for (ServiceProvider serviceProvider : oslcServiceProviderCatalog.getServiceProviders()) {
      ...
      for (final Service service : services) {
        ...
        if (Constants.FMI_VENDORTOOL_DOMAIN.equals(String.valueOf(service.getDomain()))) {
          ...
          for (final QueryCapability queryCapability : queryCapabilities) {
            ...
            final FMIVendorTool[] theFMIVendorToolArray = oslcRestClient.getOslcResources(FMIVendorTool[].class);
            
            
            if (theFMIVendorToolArray != null) {
              if (theFMIVendorToolArray.length < 1) {
                System.out.println("    No FMIVendorTool Resources");
              }
              for (final FMIVendorTool theFMIVendorTool : theFMIVendorToolArray) {
                verifyFMIVendorTool(mediaType, theFMIVendorTool);
              }
            } else { 
              System.out.println("    No FMIVendorTool Resources");
            } 
