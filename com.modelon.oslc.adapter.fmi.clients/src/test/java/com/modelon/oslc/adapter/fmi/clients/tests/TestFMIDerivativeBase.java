/*
* TestFMIDerivativeBase.java    1.1.0  2016-07-23
*  
...
public class TestFMIDerivativeBase {
  ...
  protected TestFMIDerivativeBase() throws Exception {
    super();
  }

  private static void verifyFMIDerivative(final String   mediaType, final FMIDerivative theFMIDerivative) {
    System.out.println("verifyFMIDerivative");
    assertNotNull(theFMIDerivative);
 
    final URI  aboutURI     = theFMIDerivative.getAbout();
    final String identifier  = theFMIDerivative.getIdentifier();
    final String name  = theFMIDerivative.getName();
    final String index  = theFMIDerivative.getIndex();
    final String dependencies  = theFMIDerivative.getDependencies();
    final String dependenciesKind  = theFMIDerivative.getDependenciesKind();

    System.out.println("Derivative Query Capability : " + aboutURI);
  ...
  protected void testRetrieves(final String mediaType) {
    ...
    for (ServiceProvider serviceProvider : oslcServiceProviderCatalog.getServiceProviders()) {
      ...
      for (final Service service : services) {
        ...
        if (Constants.FMI_DERIVATIVE_DOMAIN.equals(String.valueOf(service.getDomain()))) {
          ...
          for (final QueryCapability queryCapability : queryCapabilities) {
            ...
            final FMIDerivative[] theFMIDerivativeArray = oslcRestClient.getOslcResources(FMIDerivative[].class);
            
            
            if (theFMIDerivativeArray != null) {
              if (theFMIDerivativeArray.length < 1) {
                System.out.println("    No FMIDerivative Resources");
              }
              for (final FMIDerivative theFMIDerivative : theFMIDerivativeArray) {
                verifyFMIDerivative(mediaType, theFMIDerivative);
              }
            } else { 
              System.out.println("    No FMIDerivative Resources");
            } 
