...
<%--
/*
* fmiBaseUnitList.jsp    1.1.0  2016-07-24
*  
...
<%
  ...
  List<FMIBaseUnit> fmiBaseUnitList = null;
  if (identifier != null) {
    ...
    fmiBaseUnitList = Domain2OSLCManager.getFMIBaseUnitMap(fmuFileName);
  }
  String baseunitURI = request.getScheme() + "://" + request.getServerName() + ":" + request.getServerPort() + 
    "/oslc4jfmi/services/Resource/" + fmuFileName + "/BaseUnit";
%>
...
<%  if (fmiBaseUnitList != null && fmiBaseUnitList.size() > 0) { %>
      <% for (FMIBaseUnit fmiBaseUnit : fmiBaseUnitList) { %>
        <div class="col-md-12"><h4>BaseUnit : <%= Domain2OSLCManager.decodeURL(Domain2OSLCManager.getVariableName(fmiBaseUnit.getAbout().toString())) %></h4><a href="<%= fmiBaseUnit.getAbout().toString() %>"><%= fmiBaseUnit.getAbout().toString() %></a></div>
      <% } %> 
<%  } else { %> 
      <b>No resources</b>
<%  } %>
