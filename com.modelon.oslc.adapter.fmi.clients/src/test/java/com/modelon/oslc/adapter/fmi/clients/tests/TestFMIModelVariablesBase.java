/*
* TestFMIModelVariablesBase.java    1.1.0  2016-07-23
*  
...
public class TestFMIModelVariablesBase {
  ...
  protected TestFMIModelVariablesBase() throws Exception {
    super();
  }

  private static void verifyFMIModelVariables(final String   mediaType, final FMIModelVariables theFMIModelVariables) {
    System.out.println("verifyFMIModelVariables");
    assertNotNull(theFMIModelVariables);
 
    final URI  aboutURI     = theFMIModelVariables.getAbout();
    final String identifier  = theFMIModelVariables.getIdentifier();

    System.out.println("ModelVariables Query Capability : " + aboutURI);
  ...
  protected void testRetrieves(final String mediaType) {
    ...
    for (ServiceProvider serviceProvider : oslcServiceProviderCatalog.getServiceProviders()) {
      ...
      for (final Service service : services) {
        ...
        if (Constants.FMI_MODELVARIABLES_DOMAIN.equals(String.valueOf(service.getDomain()))) {
          ...
          for (final QueryCapability queryCapability : queryCapabilities) {
            ...
            final FMIModelVariables[] theFMIModelVariablesArray = oslcRestClient.getOslcResources(FMIModelVariables[].class);
            
            
            if (theFMIModelVariablesArray != null) {
              if (theFMIModelVariablesArray.length < 1) {
                System.out.println("    No FMIModelVariables Resources");
              }
              for (final FMIModelVariables theFMIModelVariables : theFMIModelVariablesArray) {
                verifyFMIModelVariables(mediaType, theFMIModelVariables);
              }
            } else { 
              System.out.println("    No FMIModelVariables Resources");
            } 
