/*
* TestFMITypeDefinitionBase.java    1.1.0  2016-07-23
*  
...
public class TestFMITypeDefinitionBase {
  ...
  protected TestFMITypeDefinitionBase() throws Exception {
    super();
  }

  private static void verifyFMITypeDefinition(final String   mediaType, final FMITypeDefinition theFMITypeDefinition) {
    System.out.println("verifyFMITypeDefinition");
    assertNotNull(theFMITypeDefinition);
 
    final URI  aboutURI     = theFMITypeDefinition.getAbout();
    final String identifier  = theFMITypeDefinition.getIdentifier();
    final String name  = theFMITypeDefinition.getName();
    final String description  = theFMITypeDefinition.getDescription();
    final String baseType  = theFMITypeDefinition.getBaseType();
    final String quantity  = theFMITypeDefinition.getQuantity();

    System.out.println("TypeDefinition Query Capability : " + aboutURI);
  ...
  protected void testRetrieves(final String mediaType) {
    ...
    for (ServiceProvider serviceProvider : oslcServiceProviderCatalog.getServiceProviders()) {
      ...
      for (final Service service : services) {
        ...
        if (Constants.FMI_TYPEDEFINITION_DOMAIN.equals(String.valueOf(service.getDomain()))) {
          ...
          for (final QueryCapability queryCapability : queryCapabilities) {
            ...
            final FMITypeDefinition[] theFMITypeDefinitionArray = oslcRestClient.getOslcResources(FMITypeDefinition[].class);
            
            
            if (theFMITypeDefinitionArray != null) {
              if (theFMITypeDefinitionArray.length < 1) {
                System.out.println("    No FMITypeDefinition Resources");
              }
              for (final FMITypeDefinition theFMITypeDefinition : theFMITypeDefinitionArray) {
                verifyFMITypeDefinition(mediaType, theFMITypeDefinition);
              }
            } else { 
              System.out.println("    No FMITypeDefinition Resources");
            } 
