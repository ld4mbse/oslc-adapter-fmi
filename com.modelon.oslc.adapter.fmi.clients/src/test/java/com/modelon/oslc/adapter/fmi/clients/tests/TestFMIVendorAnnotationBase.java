/*
* TestFMIVendorAnnotationBase.java    1.1.0  2016-07-23
*  
...
public class TestFMIVendorAnnotationBase {
  ...
  protected TestFMIVendorAnnotationBase() throws Exception {
    super();
  }

  private static void verifyFMIVendorAnnotation(final String   mediaType, final FMIVendorAnnotation theFMIVendorAnnotation) {
    System.out.println("verifyFMIVendorAnnotation");
    assertNotNull(theFMIVendorAnnotation);
 
    final URI  aboutURI     = theFMIVendorAnnotation.getAbout();
    final String identifier  = theFMIVendorAnnotation.getIdentifier();

    System.out.println("VendorAnnotation Query Capability : " + aboutURI);
  ...
  protected void testRetrieves(final String mediaType) {
    ...
    for (ServiceProvider serviceProvider : oslcServiceProviderCatalog.getServiceProviders()) {
      ...
      for (final Service service : services) {
        ...
        if (Constants.FMI_VENDORANNOTATION_DOMAIN.equals(String.valueOf(service.getDomain()))) {
          ...
          for (final QueryCapability queryCapability : queryCapabilities) {
            ...
            final FMIVendorAnnotation[] theFMIVendorAnnotationArray = oslcRestClient.getOslcResources(FMIVendorAnnotation[].class);
            
            
            if (theFMIVendorAnnotationArray != null) {
              if (theFMIVendorAnnotationArray.length < 1) {
                System.out.println("    No FMIVendorAnnotation Resources");
              }
              for (final FMIVendorAnnotation theFMIVendorAnnotation : theFMIVendorAnnotationArray) {
                verifyFMIVendorAnnotation(mediaType, theFMIVendorAnnotation);
              }
            } else { 
              System.out.println("    No FMIVendorAnnotation Resources");
            } 
