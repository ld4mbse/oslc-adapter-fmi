...
<%--
/*
* fmiModelExchangeList.jsp    1.1.0  2016-07-24
*  
...
<%
  ...
  List<FMIModelExchange> fmiModelExchangeList = null;
  if (identifier != null) {
    ...
    fmiModelExchangeList = obj;
  } else { 
    fmiModelExchangeList = Domain2OSLCManager.getFMIModelExchangeMap(fmuFileName);
  }
  String modelexchangeURI = request.getScheme() + "://" + request.getServerName() + ":" + request.getServerPort() + 
    "/oslc4jfmi/services/Resource/" + fmuFileName + "/ModelExchange";
%>
...
<%  if (fmiModelExchangeList != null && fmiModelExchangeList.size() > 0) { %>
      <% for (FMIModelExchange fmiModelExchange : fmiModelExchangeList) { %>
        <div class="col-md-12"><h4>ModelExchange : <%= Domain2OSLCManager.decodeURL(Domain2OSLCManager.getVariableName(fmiModelExchange.getAbout().toString())) %></h4><a href="<%= fmiModelExchange.getAbout().toString() %>"><%= fmiModelExchange.getAbout().toString() %></a></div>
      <% } %> 
<%  } else { %> 
      <b>No resources</b>
<%  } %>
